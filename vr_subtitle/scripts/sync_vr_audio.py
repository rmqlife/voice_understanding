...
from sense_voice.vr_sources import finished_dir, iter_finished_mp4s, local_wav_path, resolve_jav_root  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--scan-finished", action="store_true", help="Extract all VR mp4 under jav/#finished")
    ...
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "data" / "vr")
    parser.add_argument("--start", default=None, help="ffmpeg -ss")
    parser.add_argument("--duration", default=None, help="ffmpeg -t")
    ...
    if args.scan_finished:
        ...
        videos = iter_finished_mp4s(finished, vr_only=not args.all_mp4, name_filter=args.name_filter)
        if not videos:
        ...
        for video in videos:
            ...
            extract_wav_from_media(video, wav_path, start=args.start, duration=args.duration)
            print(wav_path)
        return

    if args.input is None:
        raise SystemExit("Provide --input or --scan-finished")
    output_path = args.output or args.output_dir / f"{args.input.stem}.wav"
    if args.list_only:
        print(args.input)
        print(f"  -> {output_path}")
        return
    extract_wav_from_media(args.input, output_path, start=args.start, duration=args.duration)
    print(output_path)
