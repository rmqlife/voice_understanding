...
During benchmarking, `run_asr` also had a relative-path bug: it launched ASR from the main repo root, so relative audio paths from `vr_subtitle` could be reported as missing. `run_asr` now resolves the audio path before starting the ASR subprocess.
