...
from vr_subtitle_test import DEFAULT_SRT_DIR, process_clip  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--wav-dir", type=Path, default=ROOT / "data" / "vr")
    ...
    parser.add_argument("--duration", default=None, help="Limit ffmpeg extract (for smoke tests)")
    ...
    parser.add_argument("--skip-polish", action="store_true", help="Skip LLM; .zh.srt duplicates .asr.srt (no translation)")
    parser.add_argument(
        "--repolish-only",
        action="store_true",
    ...
    parser.add_argument(
        "--no-gpu-lock",
        action="store_true",
        help="Do not wait for data/.subtitle_gpu.lock (only for debugging)",
    )
