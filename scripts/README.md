...
| `add_subtitle.sh` | — | Subtitle | `add-subtitle` 的 shell 封装 |
...
| `sync_vr_audio.py` | `sync-vr-audio` | Sync | 从 NFS `#finished` 抽 WAV → `data/vr/` |
| `sync_from_mag.sh` | `sync-results-from-mag` | Sync | 从 `rmqlife-mag` 拉 `results/`、`data/vr/` |
